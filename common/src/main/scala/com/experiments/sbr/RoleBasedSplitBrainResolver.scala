...
class RoleBasedSplitBrainResolver(stableAfter: FiniteDuration, essentialRoles: Set[String])
    extends Actor
    with ActorLogging {
    ...
    case Tick =>
      ...
      if (shouldAct) {
        ...
        if (nodesToDown.nonEmpty) {
          ...
          log.info(
            "downAction: {}, nodesToDown: [{}], downSelf: {}, (unreachable.size, members.size): {}",
            downAction,
            nodesToDown.map(_.address).mkString(", "),
            downSelf,
            (unreachable.size, members.size)
          )
