...
import com.experiments.shopping.cart.repositories.internal.{
  CassandraOffsetTrackingRepository,
  CassandraVendorBillingRepository
}
...
class AppDatabase private[repositories] (override val connector: CassandraConnection)
  ...
  protected object offsetTracking extends CassandraOffsetTrackingRepository with connector.Connector
  protected object vendorBilling extends CassandraVendorBillingRepository with connector.Connector
  val readSide = new CassandraReadSideRepository(offsetTracking, vendorBilling) with connector.Connector
}
